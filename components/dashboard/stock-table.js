...
const StockTable = ({ selectedUnit, user }) => {
  ...
  const totalCurrentStock = Object.values(unitStock).reduce((sum, qty) => sum + qty, 0);
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-200">
        ...
        {/* New Stock summary stats */}
        <div className="mt-3 grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-gray-50 p-3 rounded-lg">
            <p className="text-sm text-gray-500 mb-1">Current Stock</p>
            <p className="text-xl font-semibold text-blue-600">{totalCurrentStock}</p>
          </div>
          
          <div className="bg-gray-50 p-3 rounded-lg">
            <p className="text-sm text-gray-500 mb-1">Total Reduced</p>
            <p className="text-xl font-semibold text-orange-600">{totalReduced}</p>
          </div>
          
          <div className="bg-gray-50 p-3 rounded-lg">
            <p className="text-sm text-gray-500 mb-1">Critical Products</p>
            <p className="text-xl font-semibold text-yellow-600">
              {Object.values(unitStock).filter(qty => qty > 0 && qty <= 5).length}
            </p>
          </div>
          
          <div className="bg-gray-50 p-3 rounded-lg">
            <p className="text-sm text-gray-500 mb-1">Empty Products</p>
            <p className="text-xl font-semibold text-red-600">
              {Object.values(unitStock).filter(qty => qty === 0).length}
            </p>
          </div>
        </div>
      </div>
